...
Untrusted bot code runs in the private `bot-runner` container, never directly in the backend process. Local non-Docker development can still use Docker directly when `BOT_RUNNER_MODE=docker`.

Useful Docker variables:

```env
BOT_RUNNER_MODE=docker
