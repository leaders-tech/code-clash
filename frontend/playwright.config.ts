...
const dbPath = process.env.E2E_DB_PATH?.trim() || path.join(dbDir, `code-clash-e2e-${Date.now()}-${process.pid}.sqlite3`);
const uvCacheDir = path.join(os.tmpdir(), "code-clash-uv-cache");
